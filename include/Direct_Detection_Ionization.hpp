...
//1. Event spectra and rates
	...
	extern double R_S2_Ionization(unsigned int S2, const DM_Particle& DM, DM_Distribution& DM_distr, const Atomic_Electron& shell);
	extern double R_S2_Ionization(unsigned int S2, const DM_Particle& DM, DM_Distribution& DM_distr, const Atom& atom);
...
//2. Electron recoil direct detection experiment with isolated target atoms
	class DM_Detector_Ionization : public DM_Detector
	{
		...
		public:
			...
			void Use_Electron_Bins(unsigned int ne_thr, unsigned int N_bins = 0);
			
			void Use_S2_Bins(unsigned int ne_thr, double S2mu, double S2sigma, unsigned int N_bins = 0);
