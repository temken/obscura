...
double dRdEe_Migdal(double Ee, const DM_Particle& DM, DM_Distribution& DM_distr, Element& element, Atom& atom)
{
	double result = 0.0;
	for(auto& electron : atom.electrons)
		for(auto& isotope : element.isotopes)
			result += isotope.abundance * dRdEe_Migdal(Ee, DM, DM_distr, isotope, electron);
	return result;
}
...
// 		result += R_ne_Migdal(ne, DM, DM_distr, element, atom.electrons[i]);
// 	return result;
// }
...
// : DM_Detector("Migdal scattering experiment", kg * year, "Nuclei"), target_atom(Import_Ionization_Form_Factors("Xenon")), ne_threshold(0), ne_max(0), using_electron_threshold(false), using_electron_bins(false), PE_threshold(0), PE_max(0), S2_mu(0.0), S2_sigma(0.0), using_S2_threshold(false), using_S2_bins(false)
// {
// }

// DM_Detector_Migdal::DM_Detector_Migdal(std::string label, double expo, std::string atom)
// : DM_Detector(label, expo, "Nuclei"), target_atom(Import_Ionization_Form_Factors(atom)), ne_threshold(0), ne_max(0), using_electron_threshold(false), using_electron_bins(false), PE_threshold(0), PE_max(0), S2_mu(0.0), S2_sigma(0.0), using_S2_threshold(false), using_S2_bins(false)
// {
// }
...
// {
// 	return flat_efficiency * dRdEe_Migdal(E, DM, DM_distr, target_element, target_atom);
// }
